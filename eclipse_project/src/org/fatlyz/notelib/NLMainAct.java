...
public class NLMainAct extends Activity {
    ...
    // update thread running status
    private boolean mUpdateThdRunning = false;

    @Override
    public void onBackPressed() {
        if (llSearchBar.getVisibility() != LinearLayout.GONE) {
            llSearchBar.findViewById(R.id.common_searchbar_txtKeyword).setEnabled(false);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int _hr = t.hour; // 0-23
        if (_hr >= 1 && _hr <= 4) {
            Toast.makeText(NLMainAct.this, getString(R.string.main_time_prompt_0) + _hr + getString(R.string.main_time_prompt_1), Toast.LENGTH_LONG).show();
        }
        ...
        btnSNew.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (llSearchBar.getVisibility() == LinearLayout.VISIBLE) {
                    ...
                    llSearchBar.setVisibility(LinearLayout.GONE);
                    // Start NLCatalogAct
                    startActivity(new Intent(NLMainAct.this, NLNoteListAct.class).putExtra("LoadMode", "New"));
                    overridePendingTransition(R.anim.act_enter, R.anim.act_exit);
                }
            }
        });
            ...
            public void onClick(View v) {
                if (llSearchBar.getVisibility() == LinearLayout.VISIBLE) {
                    if (txtKeyword.getText().length() != 0) {
                        ...
                        llSearchBar.setVisibility(LinearLayout.GONE);
                        // Start NLCatalogAct
                        startActivity(new Intent(NLMainAct.this, NLNoteListAct.class).putExtra("LoadMode", "Search").putExtra("SearchKeyword",
                                txtKeyword.getText().toString()));
                        overridePendingTransition(R.anim.act_enter, R.anim.act_exit);
                    } else {
        ...
        // Create adapter
        SimpleAdapter listAdapter = new SimpleAdapter(this, listItem,// Data
                                                                     // Source
                R.layout.lstitem_main,// ListItem Layout XML
                // SubItems
                new String[] { "ItemIcon", "ItemTitle", "ItemSubtitle" }, new int[] { R.id.lstMainItemIcon, R.id.lstMainItemTitle, R.id.lstMainItemSubtitle });
        // Set adapter
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                ...
                if (arg2 == 0) { // Search
                    if (llSearchBar.getVisibility() == LinearLayout.GONE) {
                        ...
                        llSearchBar.setVisibility(LinearLayout.VISIBLE);
                        txtContact.setEnabled(false);
                        txtFeedback.setEnabled(false);
                        llFeedbackBar.setVisibility(LinearLayout.GONE);
                        txtKeyword.setText("");
                ...
                if (arg2 == 4) { // Feedback
                    if (llFeedbackBar.getVisibility() == LinearLayout.GONE) {
                        ...
                        llFeedbackBar.setVisibility(LinearLayout.VISIBLE);
    ...
    private void checkUpdate(boolean bToast) {
                    ...
                    if (getString(R.string.app_ver).equals(_latestVer)) {
                        ...
                        if (bFToast) {
                            ...
                            Toast.makeText(NLMainAct.this,
                                    getString(R.string.main_update_latest) + getString(R.string.main_updatesub) + getString(R.string.app_ver),
                        ...
                        try {
                            progDlg = new ProgressDialog.Builder(NLMainAct.this).show();
                        } catch (Throwable e) {
                            e.printStackTrace();
                            return;
                        }
                        ...
                        ((TextView) popWindow.findViewById(R.id.popDialog_txtMsg)).setText(getString(R.string.main_update_needupdate) + _latestVer
                                + getString(R.string.main_update_whetherupdate));
                        popDBtn0.setVisibility(0);
                        popDBtn0.setText(R.string.main_update_yes);
                        popDBtn1.setVisibility(0);
                        popDBtn1.setText(R.string.main_update_no);
                        popDBtn0.setOnClickListener(new OnClickListener() {
                            public void onClick(View arg0) {
                                Intent _tmpIntent = new Intent();
                                _tmpIntent.setAction("android.intent.action.VIEW");
                                _tmpIntent.setData(Uri.parse("https://api.yunzhu.org/apps/notelib/mobile/update.html"));
                                startActivity(_tmpIntent);
                                overridePendingTransition(R.anim.act_enter, R.anim.act_exit);
                            }
                        });
        ...
        new Thread() {
            public void run() {
                ...
                // dispatch data
                _msg.obj = dispatchUpdateData(mNetUtils.fetchString("https://api.yunzhu.org/apps/notelib/mobile/curversion.php?key=K06T8aDKpYJl5FEQ", new Handler()));
    ...
    private void submitFeedback(String feedback, String contact) {
        ...
        final String fFeedback = feedback;
    ...
    private int postFeedback(String feedback, String contact) {
        ...
        try {
            _r = mNetUtils.fetchString(
                    "https://api.yunzhu.org/apps/notelib/mobile/feedback.php?key=c4C864bk5FxMqHIv" + "&feedback=" + URLEncoder.encode(feedback, "UTF-8") + "&contact="
                            + URLEncoder.encode(contact, "UTF-8") + "&clientver=" + URLEncoder.encode(getString(R.string.app_ver), "UTF-8") + "&phonemodel="
                            + URLEncoder.encode(android.os.Build.BRAND + " - " + android.os.Build.MODEL, "UTF-8"), new Handler());
            mNetUtils.finalize();
