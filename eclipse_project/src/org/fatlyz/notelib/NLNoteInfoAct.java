        ...
        btnBookmark.setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                if (getSharedPreferences("NoteLibBookmarks", MODE_PRIVATE).contains((String) mNoteID) == false) {
                    ...
                    _e.putString(mNoteID + "_name", ((TextView) NLNoteInfoAct.this.findViewById(R.id.NLNoteInfoAct_txtNoteName)).getText().toString());

                    _e.putString(mNoteID + "_catanum", ((TextView) NLNoteInfoAct.this.findViewById(R.id.NLNoteInfoAct_txtCataNum)).getText().toString());

                    _e.putString(mNoteID + "_edition", ((TextView) NLNoteInfoAct.this.findViewById(R.id.NLNoteInfoAct_txtEdition)).getText().toString());

                    _e.putString(mNoteID + "_thumb_url", mNoteThumbURL);
        ...
        btnShare.setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                ...
                _tmpIntent.putExtra(Intent.EXTRA_TEXT,
                        getString(R.string.noteinfo_share_text) + ((TextView) findViewById(R.id.NLNoteInfoAct_txtNoteName)).getText()
    ...
    private void loadNoteInfo() {
        ...
        mCurThread = new Thread() {
            public void run() {
                ...
                // Fetch data string
                mDataStr = mNetUtils.fetchString("https://api.yunzhu.org/apps/notelib/mobile/noteinfo.php?key=QrB1g6fbhV9n4Oid&id=" + mNoteID, new Handler());
        ...
        mCurThread = new Thread() {
            public void run() {
                ...
                if (_noteImageFile.exists()) {
                    try {
                        mBitmap0 = BitmapFactory.decodeFile(_noteImageFile.getAbsolutePath());
                ...
                if (_noteImageFile.exists()) {
                    try {
                        mBitmap1 = BitmapFactory.decodeFile(_noteImageFile.getAbsolutePath());
                    } catch (OutOfMemoryError e) {
                    }
                    if (mBitmap1 != null) {
                        mImage1URL = null;
