...
public class NLCatalogAct extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                    ...
                    if (txtKeyword.getText().length() != 0) {
                        if (llSearchBar.getVisibility() == LinearLayout.VISIBLE) {
                            if (txtKeyword.getText().length() != 0) {
                                ...
                                llSearchBar.setVisibility(LinearLayout.GONE);
                                // Start NLCatalogAct
                                startActivity(new Intent(NLCatalogAct.this, NLNoteListAct.class).putExtra("LoadMode", "Search").putExtra("SearchKeyword",
        ...
        mELstCatalog.setOnChildClickListener(new OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView arg0, View arg1, int arg2, int arg3, long arg4) {
                ...
                startActivity(new Intent(NLCatalogAct.this, NLNoteListAct.class).putExtra("LoadMode", "Country")
                ...
                if (mLastVisitedPosCountry[0] >= 0 && mLastVisitedPosCountry[1] >= 0) {
                    ((HashMap<String, Object>) mLstCatalogAdapter.getChild(mLastVisitedPosCountry[0], mLastVisitedPosCountry[1])).put("ItemLastVisitedCountry",
                            null);
                }
                // Set new indicator
                ...
                mLastVisitedPosCountry[0] = arg2;
                mLastVisitedPosCountry[1] = arg3;
            ...
            public void run() {
                ...
                try {
                    ...
                    mDataStr = mNetUtils.fetchString(
                            "https://api.yunzhu.org/apps/notelib/mobile/countrylist.php?key=wqU4pvyuq5aSjQqR" + "&clientver="
                                    + URLEncoder.encode(getString(R.string.app_ver), "UTF-8") + "&phonemodel="
                    ...
                    /*
                     * }else if(mMode == 1){ downloadData(llHandler,
                     * "http://fatlyz.com/notelib/mobile/notelist.php?key=gEWQ41lf5z9sLx2m&country="
                     * + URLEncoder.encode(mListInCountryModeCurCountry,
                     * "UTF-8")); }else if(mMode == 2){ downloadData(llHandler,
                     * "http://fatlyz.com/notelib/mobile/notelist_newnotes.php?key=LbpzdRZwotFm2Mnc"
                     * ); }else if(mMode == 3){ downloadData(llHandler,
                     * "http://fatlyz.com/notelib/mobile/notelist_search.php?key=B3Ho1dT521lsfVg7&keyword="
                     * + URLEncoder.encode(mSearchKeyword, "UTF-8")); }
                     */
    ...
    private void dispatchCountryListData(String dataStr) {
        ...
        mLstCatalogAdapter = new NBExpandableListAdapter(
                ...
                new int[] { R.id.lstCatalogItemCID, R.id.lstCatalogItemCIcon, R.id.lstCatalogItemCTitle, R.id.lstCatalogItemCTitledata,
                        R.id.lstCatalogItemCSubtitle1, R.id.lstCatalogItem_imgLastVisitedCountry });
        ...
        for (int _ix = 1; _ix < _lineLength; _ix++) { // _ix = 1 because
            ...
            if (line[_ix] == '!') {
                ...
                if (_curField.equals("continent_sum")) {
                    mAListGroups.get(mCurContinent - 1).put("ItemTitleData", " [" + new String(line, _ix + 1, _lineLength - _ix - 2) + "]");
                } else
                // Fill data
